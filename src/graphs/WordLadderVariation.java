...
public class WordLadderVariation {
	
	public static void main(String[] args) {
		String start = "hit", end = "cog";
		ArrayList<String> dictionary = new ArrayList<String>();
		dictionary.add("hot"); dictionary.add("dot"); dictionary.add("dog"); dictionary.add("lot"); dictionary.add("log");
		System.out.println(wordLadderTwo(start, end, dictionary).toString());
	}
	
	public static ArrayList<ArrayList<String>> wordLadderTwo(String start, String end, ArrayList<String> dictionary) {
				...
				for (String word : dictionary) {
					if (!polledObj.linkedSet.contains(word) && stringDiff(word, lastWord) < 3) {
						if (stringDiff(word, end) < 3) {
							ArrayList<String> resultArr = new ArrayList<String>(polledObj.linkedSet);
							resultArr.add(word); resultArr.add(end);
